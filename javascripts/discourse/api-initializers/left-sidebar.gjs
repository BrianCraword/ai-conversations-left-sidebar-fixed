...
export default apiInitializer("2.0.3", (api) => {
  ...
  function startInlineRename(row, topicId, panel) {
    ...
    // The row is (or is wrapped by) an <a>. Per-event preventDefault proved
    // insufficient (navigation fires via a router handler we don't reliably
    // intercept). The robust fix is to neutralize the anchor entirely while the
    // editor is open: remove its href so there is nothing to navigate to, and
    // install a CAPTURE-phase click guard on the anchor that swallows clicks
    // before any router handler (which also bind in capture) can see them. Both
    // are fully restored on teardown.
    const anchor = row.closest("a") || (row.tagName === "A" ? row : row.querySelector("a"));
    const stashedHref = anchor ? anchor.getAttribute("href") : null;
    const captureGuard = (e) => {
      // Clicks INSIDE our editor (input, save, cancel) must pass through
      // untouched — this guard runs in the CAPTURE phase on the anchor, an
      // ancestor of those buttons, so calling stopPropagation here would halt
      // the event on its way DOWN and the buttons' own click handlers would
      // never fire (the "stuck editor" symptom). Do nothing for editor clicks.
      if (e.target.closest(".vc-rename-editor")) {
        return;
      }
      // Block clicks elsewhere on the (now href-less) row so nothing navigates.
      e.preventDefault();
      e.stopPropagation();
    };
    if (anchor) {
      anchor.removeAttribute("href");
      anchor.dataset.vcRenamingHref = stashedHref || "";
      anchor.addEventListener("click", captureGuard, true); // capture phase
    }
    ...
    const teardown = (newText) => {
      // Restore the anchor: re-add href and remove the capture-phase guard.
      if (anchor) {
        anchor.removeEventListener("click", captureGuard, true);
        if (stashedHref != null) {
          anchor.setAttribute("href", stashedHref);
        }
        delete anchor.dataset.vcRenamingHref;
      }
      editor.remove();
